...
|---|---|---|
| `forecast_run` | Registry of forecasting-pipeline runs — one row per run; the root that points and quality metrics tie back to. Ordered by `(forecast_run_id, started_at)`. | `forecast_run_id`, `forecast_job`, `status`, `model_name`, `model_version`, `started_at`, `finished_at` (nullable), `segments_total`, `segments_skipped` (segments with no data in the source; 0 on failed runs — `status`/`error` carry failure semantics), `error` (nullable) |
