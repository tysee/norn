...
**Инвариант: платформа, а не решение под кейс.** norn — вендор-нейтральная ПЛАТФОРМА. Доменная специфика (крипта/BTC/TON, доменные метрики/символы, ingestion, дашборды, доменные промпты и job) живёт в **инстанс-репо** (`norn-crypto-instance` — первый dogfood-инстанс, подключается submodule), а НЕ в платформенном коде (`packages/*`, `cli`). Платформа конфигурируема и доменно-агностична: никаких доменных дефолтов и хардкода (включая дефолтную метрику вроде `log_return` и доменные термины вроде `ladder`). Доменные примеры — только в тестах/доках. Найденные агентом зависимости заходят в прогноз как generic-ковариаты (TimesFM XReg), а не как крипто-специфичные ряды.
